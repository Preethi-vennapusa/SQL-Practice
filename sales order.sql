...
with cte_2003 as
(select avg(sales) as avg_2003 , to_char(order_date,'MON') as mon
from sales_order
where year_id = 2003
GROUP BY to_char(order_date,'MON')), 
cte_2004 as (select avg(sales) as avg_2004 ,to_char(order_date,'MON') as mon
from sales_order
where year_id = 2004
group by to_char(order_date,'MON'))
select yr3.mon , abs(avg_2003 - avg_2004)
from cte_2003 as yr3
join cte_2004 as yr4 on yr3.mon = yr4.mon
--solution 2
with cte as
(select year_id, avg(sales) as avg_sales , to_char(order_date,'MON') as mon
from sales_order
where year_id in ( 2003,2004)
GROUP BY year_id, to_char(order_date,'MON')
order by year_id)
select yr3.mon , abs(yr3.avg_sales - yr4.avg_sales) as diff
from cte as yr3
join cte as yr4 on yr3.mon = yr4.mon
where yr3.year_id = 2003 and yr4.year_id = 2004




select distinct case when (select avg(sales) as avg_2003
from sales_order
where year_id = 2003) >(select avg(sales) as avg_2004
from sales_order
where year_id = 2004)
then (select avg(sales) as avg_2003
from sales_order
where year_id = 2003) - (select avg(sales) as avg_2004
from sales_order
where year_id = 2004)
...
from sales_order
 where (select avg(sales) as avg_2003
from sales_order
where year_id = 2003) -

(select avg(sales) as avg_2004
from sales_order
where year_id = 2004)

select order_date()
3)Find the most profitable orders. Most profitable orders are those
whose sale price exceeded the average sale price and whose deal size was not small.
select *
from sales_order 
where deal_size <> 'Small' and sales > (select avg(sales)
from sales_order)
...
select s.order_number, s.sales, c.*
from sales_order s
join customers c on c.customer_id = s.customer
join (select c.city, avg(sales) as avg_sales
from sales_order s
join customers c on c.customer_id = s.customer
group by c.city) cte on c.city = cte.city
where s.sales> avg_sales
...
with cte as (select c.city, avg(sales) as avg_sales
from sales_order s
join customers c on c.customer_id = s.customer
group by c.city)
select s.order_number, s.sales, c.*
...
with cte  as(select c.city , avg(s.price_each) 
from sales_order s
join customers c on c.customer_id = s.customer
group by city)

join (select s.order_number ,s.price_each, c.city
from sales_order s
join customers c on c.customer_id = s.customer
where s.deal_size <> 'Small') y on y.city = cte.city
